    ...
    public:
        bool setup() {
            ...
            if (m_driver_handle == INVALID_HANDLE_VALUE) {
                printf(encrypt("failed to open driver handle. error: %x\n"), GetLastError());
                return false;
            }

            printf(encrypt("successfully opened driver handle\n"));
            return true;
        }
        ...
        bool remove_exception() {
            ...
            if (!tib) {
                printf(encrypt("failed to get tib during cleanup"));
                return false;
            }
            ...
            while (current) {
                if (current->Handler == this->m_veh_handler) {
                    printf(encrypt("found exception record at: 0x%p"), current);
                    ...
                    printf(encrypt("successfully removed exception record"));
                    return true;
                }
                prev = current;
            ...
            printf(encrypt("failed to find our exception record"));
            return false;
        }

        bool attach(std::wstring target_name) {
            //printf(hash_str("searching for process\n"));
            ...
            printf(encrypt("process id: %i\n"), m_process_id);

            m_process_handle = open_process(m_process_id);
            if (!m_process_handle)
                return false;

            //printf(hash_str("handle: %llx\n"), m_process_handle);

            m_eprocess = get_eprocess(m_process_id);
            if (!m_eprocess)
                return false;

            printf(encrypt("eprocess: %llx\n"), m_eprocess);
            ...
            printf(encrypt("found dtb: %llx, ms: %f\n"), m_dtb, time_stop.count());
        ...
        bool test_read() {
            ...
            printf(encrypt("testing read in range: 0x10,000\n"));
            for (auto i = 0ull; i < 0x10000; i++) {
                ...
                printf(encrypt("successfully read address at 0x%llx in %f ms\n"), address, time_stop.count());
                return true;
            }
        ...
        std::vector<T> read_array(uintptr_t address, size_t count) {
            ...
            if (!read_memory(address, buffer.data(), size)) {
                printf(encrypt("failed to read array of %zu elements from 0x%llX\n"), count, address);
                return {};
            }
        ...
        ret_t read(addr_t address) {
            ...
            if (!read_memory(addr, &data, sizeof(ret_t)))
                return ret_t{};

            return data;
        }

        bool read_buffer(std::uintptr_t src, void* dest, std::size_t size) noexcept {
            if (size == 0)
                return true;

            std::size_t total_read = 0;
            auto* dst_ptr = static_cast<std::uint8_t*>(dest);

            while (total_read < size) {
                const std::size_t this_chunk = min(0x1000, size - total_read);
                if (!read_memory(src + total_read, dst_ptr + total_read, this_chunk)) {
                    // failed partway through
                    return false;
                }
                total_read += this_chunk;
            }
        ...
        template<typename T>
        void batch_read(const std::vector<std::uintptr_t>& src_addrs, std::vector<T>& out_data) {
            if (src_addrs.size() != out_data.size())
                out_data.resize(src_addrs.size());

            for (size_t i = 0; i < src_addrs.size(); ++i) {
                out_data[i] = read<T>(src_addrs[i]);
            }
        }
        ...
        std::map<std::uintptr_t, std::wstring> get_process_modules() {
            ...
            do {
                ...
                if (entry.m_dll_base && module_name[0] != L'\0') {
                    ...
                    printf(encrypt("found module: %S at 0x%llX"), module_name, entry.m_dll_base);
                }
        ...
        bool is_valid_pointer(void* ptr) const {
            ...
            if (address < m_base_address || address > m_base_address + 0x10000000) return false;

            try {
                volatile std::uint8_t test = *reinterpret_cast<volatile std::uint8_t*>(ptr);
                return true;
            }
            catch (...) {
                return false;
        ...
        void debug_last_exception() const {
            printf(encrypt("driver exceptions:"));
            printf(encrypt("  - exception code: 0x%d"), m_last_error);
        }
    ...
    private:
        ...
        c_command send_command(e_command_type type, const s_command_data& data, std::uint32_t timeout_ms = 2000) const {
            ...
            if (this->m_driver_handle == INVALID_HANDLE_VALUE) {
                printf(encrypt("cannot send command: driver handle not valid\n"));
...
extern std::unique_ptr<driver::c_driver> kernel;